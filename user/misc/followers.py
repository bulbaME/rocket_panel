    ...
    with Bar('Retrieving users', max=req_count, suffix='%(percent)d%%') as bar:
        ...
        with Pool(processes=8) as pool:
            for i in range(req_count):
                pool.apply_async(get_followers_noexcept_w, (int(id), token, str(i * 100)), callback=lambda v: (data.extend(v[0]), bar.next(), err.append((v[1], str(i * 100)))))
            pool.close()
            pool.join()
        ...
        with Bar('Retrieving users', max=len(err), suffix='%(percent)d%%') as bar:
            with Pool(processes=8) as pool:
                for i in range(len(err)):
                    pool.apply_async(get_followers_noexcept_w, (int(id), token, str(i * 100)), callback=lambda v: (data.extend(v[0]), bar.next(), _err.append((v[1], str(i * 100)))))
